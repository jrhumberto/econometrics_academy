...
# note that dickey-fuller test statistic is the same as t-value in dppi ~ lppi + trend regression below, though p-value is unique
...
# because DF for differenced variable is significant, that variable is stationary and we use it for analysis

# ACF and PACF
# note that acf has a slow decay function, which is indicative of not being stationary
acf(Y)
pacf(Y)

# note that acf has a relatively fast decay function
acf(d.Y)
pacf(d.Y)
